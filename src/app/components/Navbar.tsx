    ...
    <nav className="navbar">
        ...
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center">
            ...
            <div className="hidden md:ml-8 md:flex md:space-x-6">
              <Link href="/" className={`inline-flex items-center px-1 pt-1 border-b-2 transition-colors ${pathname === '/' ? 'border-blue-500' : 'border-transparent'} ${isActive('/')}`}>
                Inicio
              </Link>
              <Link href="/dashboard" className={`inline-flex items-center px-1 pt-1 border-b-2 transition-colors ${pathname === '/dashboard' ? 'border-blue-500' : 'border-transparent'} ${isActive('/dashboard')}`}>
                Mi Dashboard
              </Link>
              <Link href="/trending" className={`inline-flex items-center px-1 pt-1 border-b-2 transition-colors ${pathname === '/trending' ? 'border-blue-500' : 'border-transparent'} ${isActive('/trending')}`}>
                Trending
              </Link>
              <Link href="/settings" className={`inline-flex items-center px-1 pt-1 border-b-2 transition-colors ${pathname === '/settings' ? 'border-blue-500' : 'border-transparent'} ${isActive('/settings')}`}>
                Configuración
              </Link>
            </div>
          </div>
      ...
      <div className={`${isMenuOpen ? 'block animate-fadeIn' : 'hidden'} md:hidden absolute w-full bg-white shadow-lg z-20`}>
        <div className="pt-2 pb-3 space-y-1 px-4">
          ...
          <Link 
            href="/settings"
            className={`block py-2 rounded-md transition-colors ${pathname === '/settings' ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-600'}`}
            onClick={() => setIsMenuOpen(false)}
          >
            Configuración
          </Link>
